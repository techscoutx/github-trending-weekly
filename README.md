# 🔥 GitHub Trending Weekly

> 每周精选 GitHub 趋势项目，深度解读，帮你发现下一个改变工作流的工具。

## 📖 为什么做这个？

每天 GitHub Trending 上涌现大量项目，但多数人只看到一行描述就划走了。

这个仓库做的事很简单：**挑出真正值得关注的项目，读完源码和文档，告诉你它到底能干什么、怎么用、值不值得试。**

## 📅 往期报告

| 日期 | 报告 | 亮点项目 |
|------|------|----------|
| 2026-09-09 | [查看报告](https://zilongguo110.github.io/github-trending-weekly/reports/2026-09-09.html) | i-have-adhd, superpowers, markitdown, hyperframes, diagram-design |

## 🔗 关注

- 🐦 Twitter/X: <!-- TODO: 填入你的推特链接 -->
- ⭐ 觉得有用？给个 Star 支持一下

## 📜 License

[MIT](LICENSE)
